...
$K^\Theta_{h(2)} << K^\Theta_{h(1)}$，一般计算pH时只考虑第一次水解。
...
$$K_{sp}^{\Theta}(A_mB_n)=\frac{\{c(A^{n+})\}^m\cdot\{c(B^{m-})\}^n}{(c^\Theta)^{m+n}}$$

#### 溶解度和溶度积的换算

对于AB型难溶性强电解质，计算：
$$c(A)\cdot c(B)=K^\Theta_{sp}(AB)(c^\Theta)^2$$

数值上满足：

$$s=\sqrt{K^\Theta_{sp}}\times c^\Theta$$

同理推得
$AB_2$ 型难溶性电解质溶度积和溶解度的关系：

$$s=\sqrt[3]{\frac{K^\Theta_{sp}}{4}}\times c^\Theta$$
